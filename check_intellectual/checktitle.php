...
if(isset($_REQUEST['pwdd'])&&$_REQUEST['pwdd']=="3lqWu0QCM0wY2i"){
	if(isset($_REQUEST['title'])&&$_REQUEST['title']!=null){
		...
		$result=$maketitle->check_title($_REQUEST['title']);
		if($result){
			echo $_REQUEST['title']." violate intellectual property";
		}else{
			echo $_REQUEST['title']." may not violate intellectual property, but you need to check by manual";
		}
	}else{
...
public function check_title($title){
	foreach($this->find as $key=>$val){
		
	$result=$this->isInString1($title,$val);
	if($result){
		echo $val.' is violate intellectual property, not use it!!';
		exit();
	}
