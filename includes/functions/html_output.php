...
// only use proportional image when image is larger than proportional size
  ...
  /*
   * The HTML out site use image wrapper function
  */
  function zen_image_out($src, $alt = '', $width = '', $height = '', $parameters = '') {
  	global $template_dir, $zco_notifier;
  	...
  	// alt is added to the img tag even if it is null to prevent browsers from outputting
  	// the image filename as default
  	$image = '<img src="http://www.amigatoy.com/' . zen_output_string($src) . '" alt="' . zen_output_string($alt) . '"';
