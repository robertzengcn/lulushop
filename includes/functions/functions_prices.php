...
// show case only
    ...
    if ($display_special_price) {
      ...
      if ($display_sale_price && $display_sale_price != $display_special_price) {
        $show_special_price = '<span class="productSpecialPriceSale" itemprop="price">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          $show_special_price = '<span class="productSpecialPrice" itemprop="price">' . '<s>' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>' . '</span>';
        } else {
          $show_special_price = '<span class="productSpecialPrice" itemprop="price">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
        }
        $show_sale_price = '';
      }
    } else {
      if ($display_sale_price) {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          ...
          $show_normal_price = '<div class="single_price" itemprop="price">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</div>';
        }
        $show_special_price = '';
        $show_sale_price = '';
      }
...
// update salemaker product prices per category per product
  ...
  function get_currencys($money){
  	global $db;

  	$currsymo="select symbol_left from " . TABLE_CURRENCIES . " where code=:money";
  	$currsymo = $db->bindVars($currsymo, ':money', $money, 'string');

  	$result=$db->Execute($currsymo);
  	return $result;
  }
