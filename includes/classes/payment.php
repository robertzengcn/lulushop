...
class payment extends base {
  ...
  */
  ...
  function create_order() {
  	if (is_array($this->modules)) {
  		if (is_object($GLOBALS[$this->selected_module])) {
  			if (method_exists($GLOBALS[$this->selected_module], 'create_order')) {
  				$GLOBALS[$this->selected_module]->create_order();
  			}
  		}
  	}
  }
  ...
  function beforemakepay($customerid,$customername,$sendto,$currency,$ordertotal,$orderip) {
  	if (is_array($this->modules)) {
  		if (is_object($GLOBALS[$this->selected_module])) {
  			if (method_exists($GLOBALS[$this->selected_module], 'beforemakepay')) {
  				return $GLOBALS[$this->selected_module]->beforemakepay($customerid,$customername,$sendto,$currency,$ordertotal,$orderip,$order_stute);
  			}
  		}
  	}
  }
  ...
  function write_log($msg,$orderid,$tradeid) {
  	if (is_array($this->modules)) {
  		if (is_object($GLOBALS[$this->selected_module]) && ($GLOBALS[$this->selected_module]->enabled) ) {
  			return $GLOBALS[$this->selected_module]->write_log($msg,$orderid,$tradeid);
  		}
  	}
  }
