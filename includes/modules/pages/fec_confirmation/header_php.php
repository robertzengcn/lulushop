...
if(isset($_POST['address'])&&($_POST['address'])!=null){
	$_SESSION['sendto']=zen_db_prepare_input((int)$_POST['address']);
}
...
$payment_modules->create_order();
$out_trade_no=$_SESSION['_alipay_order_id'];

//$out_trade_no=$payment_modules->beforemakepay($_SESSION['customer_id'],$_SESSION['customer_first_name'],$_SESSION['sendto'],$_SESSION[currency],$_SESSION[cart]->total, $_SESSION[customers_ip_address],$order->info['order_status']);
//测试产品编写
$str="";
 for ($i=0, $n=sizeof($order->products); $i<$n; $i++) { ?>
 
          
            <?php $str.= $order->products[$i]['qty'].'x  '; ?>
            <?php $str.= '商品编号：'.$order->products[$i]['model'].' '; ?>
  ...
  ?>
        <?php $str.= $order->products[$i]['attributes'][$j]['option'] . ': ' . nl2br(zen_output_string_protected($order->products[$i]['attributes'][$j]['value'])); ?>
  <?php
        } // end loop
  ...
  <?php if (sizeof($order->info['tax_groups']) > 1)  { ?>
            ...
            <?php $str.= zen_display_tax_value($order->products[$i]['tax']).'%  ' ; ?>
  <?php    }  // endif tax info display  ?>
          
            <?php $str.='商品价格:'. $currencies->display_price($order->products[$i]['final_price'], $order->products[$i]['tax'], $order->products[$i]['qty']);
            if ($order->products[$i]['onetime_charges'] != 0 ) $str.=  $currencies->display_price($order->products[$i]['onetime_charges'], $order->products[$i]['tax'], 1);
            $str.= '; ';
  ?>
...
if(isset($_POST['payment']) && $_POST['payment'] == 'alipay'){
	include(DIR_WS_MODULES . zen_get_module_directory(FILENAME_ALIPAY));
