...
// change they should do so on their account page (after creation).
    if(defined('NEWSONLY_SUBSCRIPTION_ENABLED') && (NEWSONLY_SUBSCRIPTION_ENABLED=='true')) {
      $check_subscribers_sql = "select count(customers_id) as total from " . TABLE_SUBSCRIBERS . "
							      where email_address = :email:";
      $check_subscribers_query =
       $db->bindVars($check_subscribers_sql, ':email:', $email_address, 'string');
      #$db->bindVars($check_subscribers_sql, ':email:', zen_db_input($email_address), 'string');
      ...
      if ($check_subscribers->fields['total'] > 0) {
        ...
        #$db->bindVars($sql, ':email_format:', zen_db_input($email_format), 'string');
        #$db->bindVars($sql, ':email_address:', zen_db_input($email_address), 'string');
        $db->Execute($sql);
        ...
        if (!empty($newsletter)) {
          $sql = "INSERT INTO " . TABLE_SUBSCRIBERS . " 
                  ...
                  VALUES ( :customer_id:, :email_address:, :email_format:, '1', now())";
           $sql = $db->bindVars($sql, ':customer_id:', (int)$_SESSION['customer_id'], 'integer');
           $sql = $db->bindVars($sql, ':email_address:', $email_address, 'string');
           $sql = $db->bindVars($sql, ':email_format:', $email_format, 'string');
          #$db->bindVars($sql, ':email_address:', zen_db_input($email_address), 'string');
...
// END newsletter_subscribe mod 1/1
    ...
    if (SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO_STATUS == '1' and SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO !='') {
	 ...
	 if (isset($_POST['isref'])&&$_POST['isref']==1){
